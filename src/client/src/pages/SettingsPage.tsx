...
export function SettingsPage() {
  const [activeTab, setActiveTab] = useState<"metrics" | "remoteDesktop">("metrics");
    ...
    <>
      <header className="h-16 border-b border-slate-800 flex items-center justify-between px-8 bg-slate-900 shrink-0">
        ...
        <div className="flex items-center gap-4">
            {saveStatus === "saved" && <span className="text-green-500 text-sm font-medium">Settings saved!</span>}
            <button
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-64 bg-slate-950/50 border-r border-slate-800 p-6 space-y-1">
          ...
          >
            <svg className={`w-4 h-4 mr-3 ${activeTab === "metrics" ? "text-primary-500" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          >
            <svg className={`w-4 h-4 mr-3 ${activeTab === "remoteDesktop" ? "text-primary-500" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-slate-800 border border-slate-700 rounded-lg p-5 space-y-4">
                      ...
                      <div className="space-y-2">
                        <button
                          type="button"
                          onClick={() => handleMetricsChange({ autoMode: "manual" })}
                        ...
                        <button
                          type="button"
                          onClick={() => handleMetricsChange({ autoMode: "auto" })}
                          className={`w-full flex items-start px-3 py-2 rounded-lg border text-left text-xs ${
                            currentMode === "auto"
                              ? "border-primary-500 bg-primary-500/10 text-white"
                              : "border-slate-700 bg-slate-900/40 text-slate-300 hover:border-slate-500"
                        ...
                        <button
                          type="button"
                          onClick={() => handleMetricsChange({ autoMode: "background" })}
                          className={`w-full flex items-start px-3 py-2 rounded-lg border text-left text-xs ${
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => handleMetricsChange({ refreshIntervalSeconds: Number(e.target.value) })}
                          >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => handleMetricsChange({ maxHistoryPoints: Number(e.target.value) })}
            ...
            {activeTab === "remoteDesktop" && (
                <div className="space-y-10">
                    <div className="mb-2">
                        <div className="flex items-center justify-between mb-4">
                            <div>
                                <h2 className="text-lg font-medium text-white">Remote Desktop Defaults</h2>
                                <p className="text-xs text-slate-500">
                                    Set your preferred default settings for new remote desktop sessions.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-800 border border-slate-700 rounded-lg p-5 space-y-6">
                             <div className="flex items-center justify-between p-4 bg-slate-900/50 rounded-lg border border-slate-700/50">
                                <div>
                                    <h3 className="text-sm font-semibold text-white">Auto-Resize</h3>
                                    <p className="text-xs text-slate-500">Automatically adjust remote resolution to fit your local window.</p>
                                </div>
                                <div className="relative inline-block w-10 h-5 transition duration-200 ease-in-out">
                                    <input 
                                        ...
                                        onChange={(e) => handleRemoteChange({ autoResize: e.target.checked })}
                                    />
                                    <label 
                                    ...
                                    ></label>
                                    <div className={`absolute left-1 top-1 bg-white w-3 h-3 rounded-full transition-transform duration-200 ${localRemote.autoResize ? 'translate-x-5' : 'translate-x-0'}`}></div>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="block text-xs font-medium text-slate-400">Preferred Encoder</label>
                                    <select 
                                        value={localRemote.preferredEncoder}
                                        onChange={(e) => handleRemoteChange({ preferredEncoder: e.target.value })}
                                    ...
                                    <p className="text-[10px] text-slate-500">
                                        The agent will try to use this encoder if available. Auto will select the best GPU encoder.
                                    </p>
                                </div>

                                <div className="space-y-2">
                                    <label className="block text-xs font-medium text-slate-400">Target FPS</label>
                                    <select 
                                        value={localRemote.initialFps}
                                        onChange={(e) => handleRemoteChange({ initialFps: parseInt(e.target.value) })}
                                        className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-primary-500"
                                    >
                                ...
                                <div className="space-y-2">
                                    <label className="block text-xs font-medium text-slate-400">Bitrate</label>
                                    <select
                                        value={localRemote.initialBitrateKbps}
                                        onChange={(e) => handleRemoteChange({ initialBitrateKbps: parseInt(e.target.value) || 4000 })}
                                        className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-primary-500"
                                    >
                                ...
                                <div className="space-y-2">
                                    <label className="block text-xs font-medium text-slate-400">Default Resolution</label>
                                    <div className="grid grid-cols-2 gap-2">
                                        <input 
                                            type="number" 
                                            placeholder="Width"
                                            value={localRemote.initialWidth}
                                            onChange={(e) => handleRemoteChange({ initialWidth: parseInt(e.target.value) || 1920 })}
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => handleRemoteChange({ initialHeight: parseInt(e.target.value) || 1080 })}
                                            className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-primary-500 disabled:opacity-50"
                                            disabled={localRemote.autoResize}
                                        />
                                    </div>
