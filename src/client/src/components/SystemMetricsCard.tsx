...
export function SystemMetricsCard({
  ...
  const metricsJobs = jobs.filter((j) => j.command === COLLECT_METRICS_COMMAND);
  const latestCompleted = metricsJobs.find(
    (j) => j.status === JOB_STATUS_COMPLETED && j.output,
  );
  const latestFailed = metricsJobs.find((j) => j.status === JobStatus.Failed);
            ...
            {!isOnline ? (
              ...
              <>
                ...
                <p className="text-slate-500 text-xs">
                  Click &quot;Collect metrics&quot; to run the SystemInfo module
                  on this device.
                </p>
              </>
            )}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {metrics.memoryInfo && (
                <div className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/50">
                  ...
                  <div className="text-xs text-slate-500 mt-1">
                    {formatBytesMB(metrics.memoryInfo.usedMB)} /{" "}
                    {formatBytesMB(metrics.memoryInfo.totalMB)}
                  </div>
                </div>
              )}
              ...
              {metrics.diskInfo && (
                <div className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/50">
                    ...
                    <div className="p-1.5 rounded bg-purple-500/20">
                      ...
                      >
                        <path
                          ...
                          strokeLinejoin="round"
                          strokeWidth="2"
                          d="M5 19a2 2 0 01-2-2V7a2 2 0 012-2h4a2 2 0 012 2v10a2 2 0 01-2 2H7a2 2 0 01-2-2z"
                        />
                      </svg>
                    </div>
                    <span className="text-slate-400 text-xs font-medium uppercase">
                  ...
                  <div className="text-xs text-slate-500 mt-1">
                    {formatBytesGB(metrics.diskInfo.usedGB)} used ·{" "}
                    {formatBytesGB(metrics.diskInfo.freeGB)} free
                    {metrics.diskInfo.driveName &&
                      ` (${metrics.diskInfo.driveName})`}
                  </div>
                </div>
              )}
            ...
            {metrics.memoryInfo && (
              <ProgressBar
                ...
                variant={
                  metrics.memoryInfo.usagePercent > 90
                    ? "danger"
                    : metrics.memoryInfo.usagePercent > 75
                      ? "warning"
                      : "success"
                }
              />
            ...
            {metrics.diskInfo && (
              <>
                <ProgressBar
                  ...
                  variant={
                    metrics.diskInfo.usagePercent > 90
                      ? "danger"
                      : metrics.diskInfo.usagePercent > 85
                        ? "warning"
                        : "primary"
                  }
                />
                    ...
                    {metrics.networkInfo.interfaces.slice(0, 4).map((iface) => (
                      <div key={iface.name} className="flex flex-col">
                        ...
                        <span className="truncate">
                          ↓ {formatBytesPerSecond(iface.bytesReceivedPerSecond)}{" "}
                          · ↑ {formatBytesPerSecond(iface.bytesSentPerSecond)}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                  <div className="flex justify-between py-1">
                    ...
                    <span className="text-slate-300">
                      {metrics.systemInfo.oSDescription ||
                        metrics.systemInfo.oSPlatform ||
                        "—"}
                    </span>
                  </div>
                  <div className="flex justify-between py-1">
                    ...
                    <span className="text-slate-300 font-mono text-xs">
                      {metrics.systemInfo.oSArchitecture}
                    </span>
                  </div>
                  <div className="flex justify-between py-1">
                    <span className="text-slate-500">Processors</span>
                    <span className="text-slate-300">
                      {metrics.systemInfo.processorCount}
                    </span>
                  </div>
                  <div className="flex justify-between py-1">
                  ...
                  <div className="flex justify-between py-1 col-span-2">
                    ...
                    <span className="text-slate-300 text-xs">
                      {metrics.systemInfo.frameworkDescription}
                    </span>
                  </div>
                </div>
              </div>
