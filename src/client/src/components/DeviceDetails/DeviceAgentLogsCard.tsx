...
const LEVEL_STYLES: Record<string, { badge: string; border: string }> = {
  INFO:     { badge: "text-blue-400",   border: "hover:border-l-blue-500/50" },
  WARN:     { badge: "text-yellow-400", border: "hover:border-l-yellow-500/50" },
  ERROR:    { badge: "text-red-400",    border: "hover:border-l-red-500/50" },
  CRITICAL: { badge: "text-red-600",    border: "hover:border-l-red-600/50" },
};

function levelStyle(level: string) {
  return LEVEL_STYLES[level.toUpperCase()] ?? { badge: "text-slate-400", border: "" };
}
  ...
  return (
    <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden flex flex-col h-full shadow-sm hover:shadow-md transition-shadow">
      <div className="px-6 py-5 border-b border-slate-700/50 flex items-center gap-3 bg-slate-800">
        ...
        <h3 className="text-sm font-bold text-white uppercase tracking-wider">Agent Logs</h3>
      </div>
      ...
      <div className="flex-1 bg-slate-950 p-4 font-mono text-[11px] overflow-y-auto max-h-[520px] scrollbar-dark border-t border-slate-900 shadow-inner">
        <div className="space-y-1">
          ...
          {loading && (
            <div className="px-2 py-4 text-slate-500 text-center">Loading logs...</div>
          )}

          {error && (
            <div className="px-2 py-4 text-red-400 text-center">{error}</div>
          )}

          {!loading && !error && logs.length === 0 && (
            <div className="px-2 py-4 text-slate-600 text-center">No logs yet.</div>
          )}
          ...
          {logs.map((log, i) => {
            ...
            return (
              <div
                key={i}
                className={`flex gap-4 group hover:bg-slate-900/50 px-2 py-1 rounded transition-colors border-l-2 border-transparent ${border}`}
              >
          ...
          <div className="flex gap-4 opacity-60 mt-6 px-2 py-1 border-t border-slate-800/30 pt-4">
            <span className="text-slate-600 min-w-[140px] shrink-0 font-mono">&gt;_</span>
            <span className="text-primary-400 animate-pulse font-medium">Listening for events...</span>
          </div>
        </div>
      </div>
    </div>
